class MeowsController < ApplicationController
  ...
  def toggle_like
         current_user.toggle_like!(Meow.find(params[:user_id]))
         redirect_to :back
  end
  ...
  # GET /meows/1.json
  def show
       @follow_status = current_user.follows?(@meow.user) ? 'Unfollow' : 'Follow'
  ...
  private
    ...
    def meow_params
      params.require(:meow).permit(:description, :user_id)
    end
end
